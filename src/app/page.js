  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
              ...
              <tbody>
                {filteredHistoricalData.length === 0 ? (
                  ...
                  paginatedHistoricalData.map((row, index) => (
                    <tr key={index} className="border-b border-gray-700 hover:bg-gray-700/50 cursor-pointer transition-colors" onClick={() => { setSelectedRow(row); setShowModal(true); }}>
      ...
      {showModal && selectedRow && (
        <dialog open className="fixed inset-0 z-50 backdrop:bg-black/50" style={{ backdropFilter: 'blur(4px)' }}>
          <div className="fixed inset-0 bg-black/50" onClick={() => setShowModal(false)} />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-gray-800 rounded-lg w-[90vw] max-w-5xl max-h-[90vh] shadow-2xl border border-gray-700 flex flex-col">
            {/* Dialog Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700 bg-gradient-to-r from-gray-800 to-gray-700 rounded-t-lg">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              <div className="space-y-4">
                {Object.entries(selectedRow).map(([key, value]) => (
                  <div key={key} className="flex items-start space-x-4 pb-3 border-b border-gray-700 last:border-0">
                    <span className="text-teal-400 font-semibold min-w-40 capitalize flex-shrink-0">{key.replace(/([A-Z])/g, ' $1').trim()}:</span>
                    <span className="text-gray-200 break-words flex-1">
                      {value === null || value === undefined ? (
                        <span className="text-gray-500 italic">N/A</span>
                      ) : typeof value === 'object' ? (
                        <code className="text-xs bg-gray-700 px-2 py-1 rounded block whitespace-pre-wrap">{JSON.stringify(value, null, 2)}</code>
                      ) : (
                        value.toString()
                      )}
                    </span>
            ...
            {/* Dialog Footer */}
            <div className="flex items-center justify-end px-6 py-4 border-t border-gray-700 bg-gray-800/50 rounded-b-lg gap-3">
              <button
                onClick={() => setShowModal(false)}
